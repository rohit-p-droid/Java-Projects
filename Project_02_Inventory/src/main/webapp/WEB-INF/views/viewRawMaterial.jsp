...
<%@page import="Controller.RawMaterial" %>
  ...
  <tbody>
    <tr>
      <th><%=rm.getId() %></th>
		...
		<td><a href="updateRawMaterial?id=<%=rm.getId()%>">update</a></td>
		<td><a href="issueRawMaterial?id=<%=rm.getId()%>&Rname=<%=rm.getName()%>&cost=<%=rm.getCost()%>&unit=<%=rm.getUnit()%>">issue</a></td>
    </tr>
   </tbody>
   <%
  }
